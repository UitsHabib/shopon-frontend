import React, { useEffect, useState } from "react";
import { useHistory, useLocation } from "react-router-dom";
import { useDispatch } from "react-redux";
import Dropdown from "react-bootstrap/Dropdown";

import Pagination from "../../../core/components/pagination.component";
import Table from "../../../core/components/table.component";

import { getServices } from "../service.actions";
import { useSelector } from "react-redux";
import ServiceDetails from "./serviceDetails.component";

const Services = () => {
	const history = useHistory();
    const location = useLocation();

	const dispatch = useDispatch();

	const [action, setAction] = useState({});
    const serviceData = useSelector(state => state.serviceReducer.serviceData);
    console.log(serviceData)

    const columns = [
        { label: "Title", path: "title", sort: true, content: (data, path) => <td>{data[path]}</td> },
        { label: "Slug", path: "slug", sort: true, content: (data, path) => <td>{data[path]}</td> },
        { label: "Created Date", path: "created_at", sort: true, content: (data, path) => <td>{(new Date(data.created_at)).toLocaleDateString('en-GB').replace(/\//g, '.')}</td> },
        { label: "Updated Date", path: "updated_at", sort: true, content: (data, path) => <td>{(new Date(data.updated_at)).toLocaleDateString('en-GB').replace(/\//g, '.')}</td> },
        { label: "Action", path: "slug", content: (data) => (
            <td>
                <Dropdown className="ms-auto dropdown-customize">
                    <Dropdown.Toggle
                        variant=""
                        className="btn-outline-secondary dropdown-toggle btn-sm py-0 px-1 dropdown-toggle "
                    >
                        <i className="bi bi-chevron-down fa-lg"></i>
                    </Dropdown.Toggle>

                    <Dropdown.Menu>
                        <Dropdown.Item onClick={() => setAction({ details: true, serviceId: data.id })} > Details </Dropdown.Item>
                    </Dropdown.Menu>
                </Dropdown>
            </td>
        )},
    ]

	useEffect(() => {
        if(location.search) {
            dispatch(getServices(location.search));
        } else {
            const urlSearchParams = new URLSearchParams(window.location.search);

            urlSearchParams.set('page', 1);
            urlSearchParams.set('orderBy', "title");
            urlSearchParams.set("limit", 15);
            urlSearchParams.set("orderType", "asc");

            const url = location.pathname + urlSearchParams ? `?${urlSearchParams.toString()}` : '';
            history.push(url);

            dispatch(getServices(url));
        }
	}, [action, location]);

	return (
        <div className="container-fluid">
            <div className="row">
                <div className="d-sm-flex justify-content-between align-items-center py-3">
                    <h4 className="mb-2 mb-sm-0 cdp-text-primary fw-bold mb-0 mb-sm-0 d-flex align-items-end pe-2">
                        Service list
                    </h4>
                </div>

                {serviceData['services'] && serviceData['services'].length > 0 &&
                    <div>
                        <Table 
                            columns={columns}
                            items={serviceData.services}
                        />

                        <Pagination
                            start={serviceData.meta.start}
                            end={serviceData.meta.end}
                            page={serviceData.meta.page}
                            total={serviceData.meta.total}
                            onPageChange={(searchValue) => dispatch(getServices(searchValue.url))}       
                        />
                    </div>
                }

                <ServiceDetails 
                    show={action.details}
                    serviceId={action.serviceId}
                    onHide={() => setAction({})}
                />

                {serviceData['services'] && serviceData['services'].length === 0 &&
                    <div className="row justify-content-center mt-5 pt-5 mb-3">
                        <div className="col-12 col-sm-6 py-4 bg-white shadow-sm rounded text-center">
                            <i class="icon icon-team icon-6x text-secondary"></i>
                            <h3 className="fw-bold text-primary pt-4">No Service Found!</h3>
                        </div>
                    </div>
                }
            </div>
        </div>
	);
};

export default Services;
